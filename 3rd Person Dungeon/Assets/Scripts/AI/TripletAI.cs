...
public class TripletAI : MonoBehaviour
{
    ...
    public float rangedAttackDistance = 1500f; // Дальность атаки
    ...
    private void Attack()
    {
        ...
        if (Vector3.Distance(target.transform.position, transform.position) <= rangedAttackDistance && this.gameObject.GetComponent<EnemyHP>().hp > 0 && attackDelay < 0)
        {
    ...
    private void Chase()
    {
        ...
        if (Vector3.Distance(target.transform.position, transform.position) <= seeDistance && Vector3.Distance(target.transform.position, transform.position) >= rangedAttackDistance && this.gameObject.GetComponent<EnemyHP>().hp > 0)
        {
